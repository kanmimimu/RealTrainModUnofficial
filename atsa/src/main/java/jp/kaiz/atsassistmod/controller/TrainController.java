...
/**
 * 本家 jp.kaiz.atsassistmod.controller.TrainController の移植 (編成ごとの車上装置)。
 * 速度制限 (SpeedOrder)・ATO・TASC・保安装置をまとめて tick し、最終ノッチを決定する。
 * 1.7.10 API の差分: riddenByEntity → isVehicle()/getFirstPassenger(), posX → getX() 等。
 */
public class TrainController {
    ...
    //null処理めんどいからこれとインスタンス比較で (本家コメントのまま)
    ...
    public void setTrainProtection(TrainProtectionType type) {
        ...
        if (this.train != null) {
            //表示用 (HUD / WebCTC が読む) + 再起動後の復元用
            var dataMap = this.train.getResourceState().getDataMap();
            dataMap.setString("ATSAssist_CurrentTP", type.name(), 1);
            dataMap.setInt("ATSAssist_TPType", type.id, 1);
        }
    }
    ...
    public void onUpdate() throws Exception {
        ...
        //表示用の制限速度 (HUD / WebCTC)
        int displayLimit = Math.min(this.getSpeedLimit(), this.getTrainProtectionSpeedLimit());
        var dataMap = this.train.getResourceState().getDataMap();
        dataMap.setString("ATSAssist_SpeedLimit",
                displayLimit == Integer.MAX_VALUE ? "" : String.valueOf(displayLimit), 1);
        //HUD 用のまとめ文字列 (本家 PacketTrainControllerToClient 相当を DataMap 同期で代替)。
        //"ato|tasc|limit|tpSpeed|tpName|manual" — 変化した時だけ書いて同期量を抑える。
        String hud = (this.ATO ? String.valueOf(this.getATOSpeedLimit()) : "off")
                + "|" + (this.tascController.isEnable() ? String.valueOf((int) this.tascController.getStopDistance()) : "off")
                + "|" + (this.getSpeedLimit() == Integer.MAX_VALUE ? "---" : String.valueOf(this.getSpeedLimit()))
                + "|" + (this.getTrainProtectionSpeedLimit() == Integer.MAX_VALUE ? "---" : String.valueOf(this.getTrainProtectionSpeedLimit()))
                + "|" + this.tp.getType().name()
                + "|" + (this.manualDrive ? "1" : "0");
        if (!hud.equals(dataMap.getString("ATSAssist_HUD"))) {
            dataMap.setString("ATSAssist_HUD", hud, 1);
        }
        ...
        //ControllerNotch マジもんのコントローラー用 (riddenByEntity → isVehicle)
        if (!this.train.isVehicle()) {
    ...
    private double getMovedDistance() {
        if (this.coordinates == null) {
            this.coordinates = new double[]{train.getX(), train.getY(), train.getZ()};
            return 0d;
        } else {
            //前回の座標を取得
            double[] oldXYZ = this.coordinates;
            //今回の座標
            double[] XYZ = {train.getX(), train.getY(), train.getZ()};
            //今回の座標を記録
            this.coordinates = XYZ;

            //移動距離を計算 (勾配計算ナシ — 本家のまま)
            return Math.sqrt(Math.pow((oldXYZ[0] - XYZ[0]), 2) + Math.pow((oldXYZ[2] - XYZ[2]), 2));
        }
    }
}
