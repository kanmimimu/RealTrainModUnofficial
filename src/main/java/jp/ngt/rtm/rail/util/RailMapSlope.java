...
import jp.ngt.rtm.rail.BlockLargeRailSlopeBase;
...
public class RailMapSlope extends RailMapBasic {
    ...
    public RailMapSlope(RailPosition par1, RailPosition par2, byte type) {
        super(par1, par2, 0);
        this.dirDeg = Mth.wrapDegrees((float) par1.direction * 45.0F);
        this.slopeType = type;
    }
    ...
    public void setRail(Level world, Block block, int x0, int y0, int z0, RailProperty prop) {
        this.rails.forEach(rail -> {
            ...
            if (be instanceof TileEntityLargeRailSlopeBase tile) {
